...
export function InteriorEditor({
  ...
  /*
   * אזור מכוסה בדלת אם אינו מגירה חיצונית. מגירה חיצונית היא חזית
   * בפני עצמה, ולכן אין עליה דלת ואי אפשר לצרף אותה לחזית שכנה.
   */
  const covered = (z: Zone) =>
    !zoneCells(z).every(
      (c) => c.content.kind === 'drawers' && c.content.drawerStyle !== 'inner',
    );
  const hasDoors = (unit.doors ?? 0) > 0;
  const coveredZones = zones.filter(covered);
  /** האזור התחתון שמכוסה — הוא תמיד פותח חזית, ולכן אין בו בחירה */
  const firstCovered = coveredZones[0];
  const splits = coveredZones.slice(1).map((z) => !!z.frontSplit);
  const frontMode: 'whole' | 'cell' | 'mixed' =
    splits.length === 0 || splits.every((x) => !x)
      ? 'whole'
      : splits.every((x) => x)
        ? 'cell'
        : 'mixed';

  const setFrontMode = (mode: 'whole' | 'cell') =>
    write(
      zones.map((z) => {
        const split = mode === 'cell' && covered(z) && z.id !== firstCovered?.id;
        /* חזית אחת לכל הארון מוחקת גם תא שהוגדר "בלי דלת" */
        return { ...z, frontSplit: split || undefined, doors: mode === 'whole' ? undefined : z.doors };
      }),
    );
  ...
  if (zone) {
    ...
    return (
      ...
      >
        ...
        {/*
          החזית של התא: ממשיכה את זו שמתחתיו או נפרדת ממנה. התא
          התחתון שמכוסה תמיד פותח חזית, ולכן אין בו מה לבחור.
        */}
        {hasDoors && covered(zone) && (
          <div className="mt-3">
            <span className="mb-1.5 block text-[11px] font-medium text-stone-500">החזית</span>
            <div className="flex flex-wrap gap-1.5">
              <Pill
                size="sm"
                active={!zone.frontSplit && zone.id !== firstCovered?.id}
                onClick={() => patchZone(zone.id, { frontSplit: undefined, doors: undefined })}
              >
                ממשיכה את התא שמתחת
              </Pill>
              <Pill
                size="sm"
                active={(!!zone.frontSplit || zone.id === firstCovered?.id) && zone.doors !== 0}
                onClick={() => patchZone(zone.id, { frontSplit: true, doors: undefined })}
              >
                חזית משלה
              </Pill>
              {/* נישה פתוחה בתוך ארון סגור — מדף תצוגה, מיקרוגל, ספרים */}
              <Pill
                size="sm"
                active={zone.doors === 0}
                onClick={() => patchZone(zone.id, { frontSplit: true, doors: 0 })}
              >
                בלי דלת
              </Pill>
            </div>
            {/* לחזית נפרדת יש מספר דלתות משלה; בחזית אחת הוא נקבע בעורך */}
            {(zone.frontSplit || zone.id === firstCovered?.id) &&
              zone.doors !== 0 &&
              coveredZones.length > 1 && (
              <Row label="דלתות בחזית">
                {[1, 2, 3].map((n) => (
                  <Pill
                    key={n}
                    size="sm"
                    active={(zone.doors ?? 1) === n}
                    onClick={() => patchZone(zone.id, { doors: n })}
                  >
                    {n}
                  </Pill>
                ))}
  ...
  return (
    <div className="mt-3">
          ...
          return (
            <li key={z.id}>
              <CellRow
                ...
                icon={<ZoneIcon zone={z} />}
                /* מה שמופיע כאן הוא מה שיוצא במסור: חזית משלו או המשך */
                tag={
                  hasDoors && covered(z)
                    ? z.doors === 0
                      ? 'בלי דלת'
                      : z.id === firstCovered?.id || z.frontSplit
                        ? 'חזית'
                        : 'אותה חזית'
                    : undefined
                }
      ...
      {/*
        חזית אחת לכל הארון או חזית לכל תא — וביניהן כל שילוב, כי
        דלת אחת יכולה לכסות שני תאים ותא שלישי לקבל דלת משלו. הבחירה
        כאן היא הקיצור לשני המצבים הנפוצים, והשליטה המדויקת נמצאת
        בתוך כל תא.
      */}
      {hasDoors && coveredZones.length > 1 && (
        <div className="mt-3">
          <span className="mb-1.5 flex items-baseline gap-2">
            <span className="text-[11px] font-medium text-stone-500">חזיתות</span>
            {frontMode === 'mixed' && (
              <span className="text-[10px] text-stone-400">מותאם — חזית לכמה תאים</span>
            )}
          </span>
          <div className="flex flex-wrap gap-1.5">
            <Pill size="sm" active={frontMode === 'whole'} onClick={() => setFrontMode('whole')}>
              דלת אחת לכל הארון
            </Pill>
            <Pill size="sm" active={frontMode === 'cell'} onClick={() => setFrontMode('cell')}>
              דלת לכל תא
            </Pill>
          </div>
        </div>
...
function CellRow({
  ...
  summary: string;
  locked?: boolean;
  /** מה יש בתא, בציור קטן — כדי לא להיכנס לתא רק כדי לראות */
  icon?: React.ReactNode;
  /** מה קורה בחזית של התא הזה */
  tag?: string;
  onClick: () => void;
}) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-2.5 rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-start transition-colors hover:border-oak-400"
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5 text-sm font-medium text-stone-800">
          {title}
          {locked && <LockIcon className="inline size-3 text-oak-600" />}
          {tag && (
