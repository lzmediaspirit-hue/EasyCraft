...
/**
 * מנרמל זווית לטווח (-180, 180], בדיוק שמחזיר צורה סגורה לנקודתה.
 *
 * כאן היה `Math.round`, וזה שבר חדר משורטט: משולש 4/2.5/2.5 מטר
 * נשמר בפניות 143° ו-74° במקום 143.1301° ו-73.7398°, ובניית
 * השרשרת מחדש הסתיימה 6.8 מ״מ מהנקודה שיצאה ממנה. עיגול עצמאי של
 * כל זווית אינו משמר את אילוץ הסגירה של מצולע שרירותי — והחדר
 * שהאשף קרא לו "סגור" נשמר פתוח.
 *
 * הזווית אינה מוצגת לאיש: היא נגזרת מהשרטוט ונשמרת. מה שהמשתמש
 * רואה ומקליד הוא האורך, ורק הוא מעוגל. שש ספרות אחרי הנקודה הן
 * שבע מאיות של מיקרון על קיר של ארבעה מטרים.
 */
  ...
  const footer =
    step === 'name' ? (
      ...
      <PrimaryButton disabled={drawn.length === 0 || !!crossing} onClick={() => setStep('order')}>
        {crossing
          ? crossing[2] === 'retrace'
            ? `הקיר ${crossing[1] + 1} חוזר על הקיר ${crossing[0] + 1} — זה אותו קיר פעמיים`
