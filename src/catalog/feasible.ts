...
export function unitProblem(
  ...
  /*
   * מוצר תופס מקום, ולכן יש לו רוחב, גובה ועומק.
   *
   * אפס הוא ערך תקין ברגליים, במשטח ובגובה מהרצפה — שם הוא אומר
   * "אין". בשלוש מידות הגוף הוא אומר "המוצר אינו קיים", ובכל זאת
   * עבר: לוח ומכשיר יצאו מהבדיקה לפני שהגיעו לכאן, ומדף ברוחב
   * אפס נכנס לרשימת החיתוך. הבדיקה הזאת קודמת לכל הסתעפות לפי
   * סוג המוצר, כי היא נכונה לכולם.
   */
  ...
  const empty = ZERO.find(([, mm]) => mm <= 0);
  if (empty) return `${empty[0]} חייב להיות גדול מאפס.`;
