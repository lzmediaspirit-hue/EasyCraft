/*
 * `ApplianceType` יושב בקטלוג ולא כאן: מידות התקן של מכשיר חשמלי
 * הן ידיעה מקצועית על הציוד, ולא חלק ממודל הנתונים. הקובץ שם אינו
 * מייבא דבר, ולכן אין כאן מעגל.
 */
...
export interface Room extends Entity {
   ...
   *
   * אופציונלי: חדר שנוסף בלי תיאור לא שומר מחרוזת ריקה. שדה ריק
   * ושדה חסר הם אותו דבר, ושמירה של הריק פסלה את הקובץ בייבוא.
   */
  hint?: string;
  /** מפתח האייקון */
  icon: string;
  ...
  groups: CatalogGroup[];
  /**
   * לפי איזה פרופיל החדר הזה מתוכנן אוטומטית.
   *
   * זהות החדר ופרופיל התכנון שלו אינם אותו דבר, וזה בדיוק מה
   * שנשבר בייבוא: הספרייה של הבעלים נשמרה עם מזהי UUID ישנים,
   * ולכן אחרי ייבוא היו שתי "אמבטיה" — אחת מובנית עם פרופיל
   * ובלי ארגזים, ואחת מיובאת עם שבעה ארגזים ובלי פרופיל.
   *
   * ריק = החדר מתוכנן לפי המזהה שלו, וזו ברירת המחדל לחדרים
   * המובנים. חדר שהנגר הוסיף בעצמו יכול להצביע כאן על פרופיל
   * קיים, וכך לקבל תכנון אוטומטי בלי לאבד את זהותו.
   */
  plannerProfile?: RoomKind;
  sortOrder: number;
  isBuiltin: boolean;
  /** הוסתר מרשימת החדרים */
  hiddenAt?: number;
...
export interface PlacedUnit extends Entity {
  ...
  glyph: string;
  /**
   * סוג המכשיר שהארגז מיועד לו, בנפרד מהאיור.
   *
   * האיור הוא תמונה קטנה ברשימה; הסוג הוא מה שקובע איזו נישה
   * נדרשת ובאילו מידות תקן. מי שצייר "פתוח" וכתב שזו נישת תנור
   * מקבל נישת תנור, ומי שהחליף איור בלי לגעת במבנה אינו מאבד
   * את היכולת. ריק = נגזר מהאיור, וזו ברירת המחדל.
   */
  ...
  zones?: Zone[];
  /**
   * כמה הארגז נמשך מהקיר אל תוך החדר, במ"מ.
   *
   * ארגז צמוד קיר לא יכול היה לזוז בניצב לו כלל: הדרך היחידה
   * להרחיק אותו מהקיר הייתה להפוך אותו לאי, וזה שינוי אחר לגמרי
   * — הוא מאבד את הקיר שהוא נמדד לפיו, את הפינה שהוא נצמד אליה
   * ואת מקומו בשורה. כאן הוא נשאר ארגז על הקיר ורק עומד רחוק
   * ממנו: קיר לא ישר, טיח, או פאנל שמאחוריו.
   *
   * ריק = צמוד, וזה המצב הרגיל.
   */
  offWallMm?: number;
  /** מנגנון פתיחת החזית */
  opening?: OpeningMech;
...
export interface CatalogItem extends Entity {
   ...
   * אין כאן `hiddenAt`.
   *
   * פעם הסרה מהספרייה רק סימנה את הפריט, והוא נשאר במכשיר: לא
   * הופיע ברשימות אבל יצא בכל גיבוי וחזר עם הספרייה למכשיר הבא.
   * המעבר בגרסה 23 מחק אותם בפועל, ומאז שום מסך אינו כותב את
   * השדה — נשארו רק שלושה סינונים ששמרו מפני מה שכבר אינו קיים.
   * מה שמוסר, מוסר. פריט שמגיע מקובץ ישן עם הסימון הזה מנוקה
   * בייבוא — ראה `db/legacy`.
   */
  sortOrder: number;
  /** הערת תקן קצרה — למה המידות האלה */
  note?: string;
  /**
   * טביעת האצבע של התבנית כפי שהיא הגיעה בשחרור האחרון.
   *
   * בלעדיה אי אפשר להבדיל בין תבנית שהנגר שינה לבין תבנית
   * שהשחרור שינה: שתיהן "שונות ממה שנשלח". מי שנושא את הסימון
   * ותוכנו זהה לו — לא נגעו בו, ואפשר לעדכן אותו בשקט; מי
   * שתוכנו שונה ממנו נערך כאן, והעדכון שלו נשאל במפורש.
   *
   * ריק = הגיע לפני שהמנגנון היה, ולכן הוא נשאל תמיד.
   */
  releaseMark?: string;
  /**
...
export interface Settings {
  ...
  /**
   * איזו גרסת ספרייה הנגרייה כבר קיבלה במפורש.
   *
   * שינוי `SHIPPED_LIBRARY` אינו מגיע להתקנה שכבר סומנה כנזרעה:
   * הזריעה רצה פעם אחת, וזה נכון — אסור לה למחוק התאמות אישיות
   * ולהחזיר מה שנמחק בכוונה. אבל המשמעות היא שתבנית מתוקנת אינה
   * מגיעה לאף אחד מהקיימים.
   *
   * המספר הזה הוא מה שמפריד בין השניים: הוא אומר איזו גרסה כבר
   * נראתה, והעדכון מציג מה נוסף, מה השתנה ומה נמחק — ומחיל רק
   * את מה שהנגר אישר. ראה `catalog/libraryRelease.ts`.
   */
  libraryRelease?: number;
  /**
   * איזה דור של הספרייה שמגיעה עם האפליקציה הנגרייה הזאת כבר קיבלה.
   *
   * `libraryRelease` הוא מנגנון ההצעה, והוא נכון לשינוי נקודתי:
   * תבנית שהשתנתה מוצגת, והנגרייה בוחרת. החלפה של ספרייה שלמה
   * באחרת אינה שינוי נקודתי — אין מה להציע ואין מה למזג, כי מה
   * שהיה כאן אינו הגרסה הישנה של מה שבא אלא פשוט משהו אחר.
   *
   * המספר הזה אומר איזו ספרייה נזרעה כאן. כשהוא נמוך מהדור שבקוד,
   * הספרייה שהגיעה עם האפליקציה מוחלפת כולה. ראה
   * `catalogRepo.replaceLibrary`.
   */
...
export interface ProjectDefaults {
   ...
   * עובי המשטח.
   *
   * שני המספרים יחד קובעים את גובה הגוף: ראש המשטח יושב בגובה
   * העבודה, ולכן הגוף הוא ההפרש. בלי העובי גובה העבודה לבדו אינו
   * מספיק — נגרייה שעברה ממשטח 2 ס״מ ל-3 בנתה ארון גבוה בסנטימטר.
   */
  counterMm: number;
