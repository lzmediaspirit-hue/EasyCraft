/* שכבה 21 — הסתרת ארגזים, בלי עליונים, ומחווני החדר */
...
await page.screenshot({ path: 'L59-1-hidden.png' });
...
/* --- בלי עליונים: מוריד את התלויים בלבד --- */
await addUnit(page, 3);
await page.waitForTimeout(900);
/* הארגז השני נתלה על הקיר — כך יש מה להוריד */
...
await btn(/בלי עליונים/).click();
await page.waitForTimeout(800);
ok(
  'no-uppers drops exactly the hanging cabinets',
  (await drawn()) === all - uppers,
  `${all} - ${uppers} = ${await drawn()}`,
);
await page.screenshot({ path: 'L59-2-no-uppers.png' });
await btn(/בלי עליונים/).click();
await page.waitForTimeout(700);
ok('and brings them back', (await drawn()) === all);
...
await page.screenshot({ path: 'L59-3-room-stats.png' });
