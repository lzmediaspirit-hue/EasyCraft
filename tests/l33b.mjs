...
const add = async (re) => {
  ...
  await dlg().getByRole('button', { name: /^סלון/ }).click(); await page.waitForTimeout(600);
  await dlg().getByRole('button', { name: 'ארונות', exact: true }).click(); await page.waitForTimeout(500);
...
await add(/פאנל חיפוי קיר/);
await add(/מזנון גבוה/);
...
const cab = u.find((z) => z.g !== 'panel');
const panel = u.find((z) => z.g === 'panel');
...
u = await readUnits();
const a = u.find((z) => z.g === 'panel');
const c = u.find((z) => z.g !== 'panel');
